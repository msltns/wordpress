 ...
 */
if ( ! class_exists( '\msltns\wordpress\MS_Update_Manager' ) ) {
    ...
    class MS_Update_Manager {
    	...
    	public function __construct( $_api_url, $_plugin_file, $_api_data = null ) {
    		...
    		$this->prefix                   = $_api_data['prefix'];
    		$this->slug                     = basename( dirname( $_plugin_file ) );
    		$this->version                  = $_api_data['version'];
    		$this->wp_override              = isset( $_api_data['wp_override'] ) ? (bool) $_api_data['wp_override'] : false;
    		$this->beta                     = ! empty( $this->api_data['beta'] ) ? true : false;
    		$this->failed_request_cache_key = "msltns_{$this->prefix}_failed_http_" . md5( $this->api_url );
    	...
    	public function check_update( $_transient_data ) {
            ...
            if ( false !== $current && is_object( $current ) && isset( $current->version ) ) {
    			if ( version_compare( $this->version, $current->version, '<' ) ) {
    	...
    	private function get_update_transient_data() {
    		...
    		$limited_data->download_url = $version_info->download_url;
            $limited_data->package      = $version_info->download_url; //$version_info->package;
    		...
    		$limited_data->version      = $version_info->version;
    		$limited_data->new_version  = $version_info->version;
    	...
    	public function show_update_notification( $file, $plugin ) {
    		...
    		if ( empty( $update_cache->response[ $this->name ] ) || version_compare( $this->version, $update_cache->response[ $this->name ]->version, '>=' ) ) {
    			return;
    		}
    		...
    		printf(
    			...
    			esc_html__( 'There is a new version of %1$s available.', 'msltns' ),
    			esc_html( $plugin['Name'] )
    		);

    		if ( ! current_user_can( 'update_plugins' ) ) {
    			echo ' ';
    			esc_html_e( 'Contact your network administrator to install the update.', 'msltns' );
    		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
    			echo ' ';
    			printf(
    				/* translators: 1: opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
    				__( '%1$sView version %2$s details%3$s.', 'msltns' ),
    				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
    				esc_html( $update_cache->response[ $this->name ]->version ),
    				'</a>'
    			);
    		} elseif ( ! empty( $changelog_link ) ) {
    			echo ' ';
    			printf(
    				__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'msltns' ),
    				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
    				esc_html( $update_cache->response[ $this->name ]->version ),
    				'</a>',
    	...
    	public function plugins_api_filter( $_data, $_action = '', $_args = null ) {
    		...
    		$msltns_api_request_transient = $this->get_cached_version_info();

    		// If we have no transient-saved value, run the API, set a fresh transient with the API value, and return that value too right now.
    		if ( empty( $msltns_api_request_transient ) ) {
    			...
    			$_data = $msltns_api_request_transient;
    		}
    		...
    		if ( ! isset( $_data->version ) && ! empty( $_data->version ) ) {
    			$_data->version = $_data->version;
    		}
    	...
    	public function http_request_args( $args, $url ) {
    		if ( strpos( $url, 'https://' ) !== false && strpos( $url, 'msltns_action=package_download' ) ) {
    	...
    	public function show_changelog() {
    		...
    		if ( empty( $_REQUEST["msltns_{$this->prefix}_action"] ) || 'view_plugin_changelog' !== $_REQUEST["msltns_{$this->prefix}_action"] ) {
    			return;
    		}
    		...
    		if ( ! current_user_can( 'update_plugins' ) ) {
    			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'msltns' ), esc_html__( 'Error', 'msltns' ), array( 'response' => 403 ) );
    		}
    	...
    	private function get_version_from_remote() {
            ...
            if ( is_wp_error( $response ) ) {
    			...
    			$this->log_failed_request();
                $this->log( 'response code: ' . wp_remote_retrieve_response_code( $response ), 'error' );
    			return false;
    		}
    	...
    	public function set_version_info_cache( $value = '', $cache_key = '' ) {
            ...
            // delete_option( 'msltns_api_request_' . md5( serialize( $this->slug . $this->api_data['license'] . $this->beta ) ) );
    	}
    	...
    	private function verify_ssl() {
    		return (bool) apply_filters( "msltns_{$this->prefix}_api_request_verify_ssl", true, $this );
    	}
